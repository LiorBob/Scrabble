...
namespace Scrabble
{
    class NonFirstMove
    {
        ...
        private void InvokeNonFirstMove()
        {
            ...
            else                   // At least 1 incorrect word
            {
        ...
        private bool CheckWordPutCorrectly()
        {
            ...
            {
                wordPutOnBoard.Sort(delegate(Button b1, Button b2) { return b1.Location.X.CompareTo(b2.Location.X); });
            ...
            {
                wordPutOnBoard.Sort(delegate(Button b1, Button b2) { return b1.Location.Y.CompareTo(b2.Location.Y); });
        ...
        private int GetSquareIndex(Button button)
        {
            ...
            x = button.Location.X - lblBoardSquares[0].Location.X;
            y = button.Location.Y - lblBoardSquares[0].Location.Y;
        ...
        private int GetWordPoints(string word, int i)
        {
            ...
            foreach (Button button in wordPutOnBoard)
            {
                ...
                {
                    squaresBonuses.TryGetValue(button.Location, out bonus);
                    ...
                    switch (bonus)
                    {
                        ...
                        case "2L":
                            ...
                            {
                                wordPoints += letter.GetPoints(button.Text[0]);
                                toolTipLetter.Show(letter.GetPoints(button.Text[0]) + "x2", button, 1, 1, delay);
                            }

                            else         // white button :  0 points
                            {
                                toolTipLetter.Show("0x2", button, 1, 1, delay);
                            }
                        ...
                        case "3L":
                            ...
                            if (button.BackColor != Color.White)
                            {
                                wordPoints += letter.GetPoints(button.Text[0]) * 2;
                                toolTipLetter.Show(letter.GetPoints(button.Text[0]) + "x3", button, 1, 1, delay);
                            }

                            else         // White button :  0 points
                            {
                                toolTipLetter.Show("0x3", button, 1, 1, delay);
                            }
